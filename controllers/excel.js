const { request, response } = require("express");
const xl = require("excel4node");
const path = require("path");
const https = require("https");

const excel = (req = request, res = response) => {
  //const arrayData = req.body;
  const arrayData = [
    {
      "id": "Id",
      "name": "Nombre",
      "username": "Usuario",
      "email": "Email",
      "street": "Calle",
      "suite": "Dirección",
      "city": "Ciudad",
      "zipcode": "Código Postal",
      "phone": "Telefono",
      "website": "Sitio Web",
      "companyname": "Nombre de Empresa"
    },
    {
      "id": "1",
      "name": "Leanne Graham",
      "username": "Bret",
      "email": "Sincere@april.biz",
      "street": "Kulas Light",
      "suite": "Apt. 556",
      "city": "Gwenborough",
      "zipcode": "92998-3874",
      "phone": "1-770-736-8031 x56442",
      "website": "hildegard.org",
      "companyname": "Romaguera-Crona"
    },
    {
      "id": "2",
      "name": "Ervin Howell",
      "username": "Antonette",
      "email": "Shanna@melissa.tv",
      "street": "Victor Plains",
      "suite": "Suite 879",
      "city": "Wisokyburgh",
      "zipcode": "90566-7771",
      "phone": "010-692-6593 x09125",
      "website": "anastasia.net",
      "companyname": "Deckow-Crist"
    },
    {
      "id": "3",
      "name": "Clementine Bauch",
      "username": "Samantha",
      "email": "Nathan@yesenia.net",
      "street": "Douglas Extension",
      "suite": "Suite 847",
      "city": "McKenziehaven",
      "zipcode": "59590-4157",
      "phone": "1-463-123-4447",
      "website": "ramiro.info",
      "companyname": "Romaguera-Jacobson"
    },
    {
      "id": "4",
      "name": "Patricia Lebsack",
      "username": "Karianne",
      "email": "Julianne.OConner@kory.org",
      "street": "Hoeger Mall",
      "suite": "Apt. 692",
      "city": "South Elvis",
      "zipcode": "53919-4257",
      "phone": "493-170-9623 x156",
      "website": "kale.biz",
      "companyname": "Robel-Corkery"
    },
    {
      "id": "5",
      "name": "Chelsey Dietrich",
      "username": "Kamren",
      "email": "Lucio_Hettinger@annie.ca",
      "street": "Skiles Walks",
      "suite": "Suite 351",
      "city": "Roscoeview",
      "zipcode": "33263",
      "phone": "(254)954-1289",
      "website": "demarco.info",
      "companyname": "Keebler LLC"
    },
    {
      "id": "6",
      "name": "Mrs. Dennis Schulist",
      "username": "Leopoldo_Corkery",
      "email": "Karley_Dach@jasper.info",
      "street": "Norberto Crossing",
      "suite": "Apt. 950",
      "city": "South Christy",
      "zipcode": "23505-1337",
      "phone": "1-477-935-8478 x6430",
      "website": "ola.org",
      "companyname": "Considine-Lockman"
    },
    {
      "id": "7",
      "name": "Kurtis Weissnat",
      "username": "Elwyn.Skiles",
      "email": "Telly.Hoeger@billy.biz",
      "street": "Rex Trail",
      "suite": "Suite 280",
      "city": "Howemouth",
      "zipcode": "58804-1099",
      "phone": "210.067.6132",
      "website": "elvis.io",
      "companyname": "Johns Group"
    },
    {
      "id": "8",
      "name": "Nicholas Runolfsdottir V",
      "username": "Maxime_Nienow",
      "email": "Sherwood@rosamond.me",
      "street": "Ellsworth Summit",
      "suite": "Suite 729",
      "city": "Aliyaview",
      "zipcode": "45169",
      "phone": "586.493.6943 x140",
      "website": "jacynthe.com",
      "companyname": "Abernathy Group"
    },
    {
      "id": "9",
      "name": "Glenna Reichert",
      "username": "Delphine",
      "email": "Chaim_McDermott@dana.io",
      "street": "Dayna Park",
      "suite": "Suite 449",
      "city": "Bartholomebury",
      "zipcode": "76495-3109",
      "phone": "(775)976-6794 x41206",
      "website": "conrad.com",
      "companyname": "Yost and Sons"
    },
    {
      "id": "10",
      "name": "Clementina DuBuque",
      "username": "Moriah.Stanton",
      "email": "Rey.Padberg@karina.biz",
      "street": "Kattie Turnpike",
      "suite": "Suite 198",
      "city": "Lebsackbury",
      "zipcode": "31428-2261",
      "phone": "024-648-3804",
      "website": "ambrose.net",
      "companyname": "Hoeger LLC"
    }
  ];
  
  let pathExcel = "";

  try {

    const wb = new xl.Workbook();
    const ws = wb.addWorksheet("Sheet");

    const headerStyle = wb.createStyle({
      alignment: {
        wrapText: true,
        horizontal: "center",
      },
      font: {
        bold: true,
        size: 12,
      },
      border: {
        left: {
          style: "thin",
        },
        right: {
          style: "thin",
        },
        top: {
          style: "thin",
        },
        bottom: {
          style: "thin",
        },
      },
      fill: {
        type: "pattern",
        patternType: "solid",
        bgColor: "gray-40",
        fgColor: "gray-40",
      },
    });

    const dataStyle = wb.createStyle({
      alignment: {
        wrapText: true,
        horizontal: "center",
      },
      font: {
        size: 12,
      },
      border: {
        left: {
          style: "thin",
        },
        right: {
          style: "thin",
        },
        top: {
          style: "thin",
        },
        bottom: {
          style: "thin",
        },
      },
    });

    if (arrayData.length) {
      arrayData.forEach((item, y) => {
        const style = !y ? headerStyle : dataStyle;
        const arrayValues = Object.values(item);
        for (let x = 0; x < arrayValues.length; x++) {
          if (typeof arrayValues[x] === "string") {
            ws.cell(y + 1, x + 1)
              .string(arrayValues[x])
              .style(style);
          } else {
            throw "Only string values are allowed";
          }
        }
      });
    } else {
      throw "The data request is empty";
    }

    wb.writeToBuffer().then((buffer) => {
      const filename = "DataExcel.xlsx";
      res
        .set(
          "content-disposition",
          `attachment; filename="${filename}";  filename*=UTF-8''${encodeURI(
            filename
          )}`
        )
        .type(".xlsx")
        .send(buffer);
    });

  } catch (err) {
    return res.status(500).json({
      error: true,
      statusCode: 500,
      msg: err,
    });
  }
};

module.exports = {
    excel
};
